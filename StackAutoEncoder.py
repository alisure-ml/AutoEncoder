...
class MnistData:
    ...
    def __init__(self):
        # 手写体数据
        self._mnist = input_data.read_data_sets("data/", one_hot=False)
        # 初始数据
...
class Runner:
    ...
    def train(self, train_layer_1_epochs, train_layer_2_epochs, train_classifies_epochs, batch_size=64):
        ...
        for epoch in range(train_layer_1_epochs):
            ...
            for i in range(total_batch):
                ...
                cost, _ = self.auto_encoder.sess.run([self.auto_encoder.layer_1_cost, self.auto_encoder.layer_1_optimizer], feed_dict={self.auto_encoder.layer_1_x: batch_xs})
        ...
        self.data.x_train_input = self.auto_encoder.sess.run(self.auto_encoder.layer_1_hidden, feed_dict={self.auto_encoder.layer_1_x: self.data.x_train_input})
        ...
        for epoch in range(train_layer_2_epochs):
            ...
            for i in range(total_batch):
                batch_xs, _ = self.data.nest_batch_train(batch_size)
                cost, _ = self.auto_encoder.sess.run([self.auto_encoder.layer_2_cost, self.auto_encoder.layer_2_optimizer], feed_dict={self.auto_encoder.layer_2_x: batch_xs})
                avg_cost += cost / self.data.train_number * batch_size
            self.print_info("{} cost: {}".format(epoch + 1, avg_cost))
            pass
        ...
        self.data.x_train_input = self.auto_encoder.sess.run(self.auto_encoder.layer_2_hidden, feed_dict={self.auto_encoder.layer_2_x: self.data.x_train_input})
        ...
        for epoch in range(train_classifies_epochs):
            ...
            for i in range(total_batch):
                ...
                _, _, result = self.auto_encoder.sess.run([self.auto_encoder.classifies_cost, self.auto_encoder.classifies_optimizer, self.auto_encoder.classifies_result],
                ...
                pass
            self.print_info("after {} accuracy: {} ({}/{})".format(epoch + 1, accuracy / (total_batch * batch_size), accuracy, total_batch * batch_size))
            pass
        self.print_info("classifies layers train end")

        # 对分类进行测试
        self.print_info("begin to test classifies layers")
        result = self.auto_encoder.sess.run(self.auto_encoder.classifies_result, feed_dict={self.auto_encoder.classifies_x: self.data.x_train_input})
        accuracy_number = self.stat_accuracy(result, self.data.y_train)
        self.print_info("now accuracy is {} ({}/{})".format(accuracy_number / self.data.train_number, accuracy_number, self.data.train_number))
    ...
    def fine_tuning(self, fine_epochs, batch_size=64):
        ...
        for epoch in range(fine_epochs):
            ...
            for i in range(total_batch):
                ...
                cost, _, result = self.auto_encoder.sess.run([self.auto_encoder.last_cost, self.auto_encoder.last_optimizer, self.auto_encoder.last_result],
                                                             feed_dict={self.auto_encoder.layer_1_x: batch_xs, self.auto_encoder.classifies_y: batch_ys})
                avg_cost += cost / self.data.train_number * batch_size
                accuracy += self.stat_accuracy(result, batch_ys)
            self.print_info("{} cost: {}".format(epoch + 1, avg_cost))
            self.print_info("now accuracy is {} ({}/{})".format(accuracy / (total_batch * batch_size), accuracy, total_batch * batch_size))
            pass
        self.print_info("fine tuning end")

        # 测试
        self.print_info("begin to test")
        result = self.auto_encoder.sess.run(self.auto_encoder.last_result, feed_dict={self.auto_encoder.layer_1_x: self.data.x_train})
        accuracy_number = self.stat_accuracy(result, self.data.y_train)
        self.print_info("now accuracy is {} ({}/{})".format(accuracy_number / self.data.train_number, accuracy_number, self.data.train_number))
    ...
    def test(self):
        self.print_info("begin to test")
        result = self.auto_encoder.sess.run(self.auto_encoder.last_result, feed_dict={self.auto_encoder.layer_1_x: self.data.x_test})
        accuracy_number = self.stat_accuracy(result, self.data.y_test)
        self.print_info("now accuracy is {} ({}/{})".format(accuracy_number / self.data.test_number, accuracy_number, self.data.test_number))
        self.print_info("test end")
        pass
...
def train_test_fine_test(layer_1_n=200, layer_2_n=50, layer_1_epochs=20, layer_2_epochs=10, classifies_epochs=10, fine_epochs=100):
